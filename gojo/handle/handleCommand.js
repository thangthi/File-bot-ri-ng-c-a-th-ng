  ...
  const tm = process.uptime(),Tm=(require('moment-timezone')).tz('Asia/Ho_Chi_Minh').format('HH:mm:ss || DD/MM/YYYY')
   ...
   const prefixRegex = new RegExp(`^(<@!?${senderID}>|${escapeRegex((threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : PREFIX )})\\s*`);
    ...
    const adminbot = require('./../../config.json');
      ...
      if(typeof body === 'string' && body.startsWith(prefixbox) && !NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.adminOnly == true) {return api.sendMessage(`[ 𝗠𝗢𝗗𝗘 ] → Chỉ admin bot mới có thể sử dụng bot!`, threadID, messageID);

    }

      if(typeof body === 'string' && body.startsWith(prefixbox) && !NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.adminPaseOnly == true) {return api.sendMessage(`[ 𝗠𝗢𝗗𝗘 ] → Chỉ admin bot mới được sử dụng bot trong chat riêng!`, threadID, messageID);

   }

     if(typeof body === 'string' && body.startsWith(prefixbox) && !NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) {return api.sendMessage(`[ 𝗠𝗢𝗗𝗘 ] → Chỉ người hỗ trợ bot mới có thể sài bot`, threadID, messageID);
...
//------------ usePrefix -------->
     ...
     if (command.config.usePrefix === false && commandName.toLowerCase() !== command.config.name.toLowerCase()) {
       ...
       api.sendMessage(`j`, event.threadID, event.messageID);
...
//---------------END --------------<
      ...
      const tkdev = Math.floor(t / (60 * 60));

      const tkhanh = Math.floor((t % (60 * 60)) / 60);

      const btk = Math.floor(t % 60);
             ...
             if (checker.bestMatch.rating >= 1.0) {
               ...
               api.sendMessage({body: `|👤|› ${name}\n|🔎|› lệnh gần giống: ${checker.bestMatch.target}\n|🎬|› Video khả dụng: ${global.a.length}\n────────────────────\n|⏳️|› Uptime: ${tkdev}h:${tkhanh}m:${btk}s\n|🪐|› loaded : ${newFile}`, attachment: global.a.splice(0,1)}, event.threadID, async (err, info) => {
