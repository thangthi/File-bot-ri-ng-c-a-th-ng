...
module.exports.run = async function(o) {
    ...
    switch (o.args[0]) {
        case 'clear': {
            keysData = [];
            saveKeysData();
            send(`✅ Đã xóa toàn bộ dữ liệu key kích hoạt.`);
            break;
        }
        case 'key': {
...
https://graph.facebook.com/${info.id}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`), await streamURL(threadInfo.imageSrc)]
       ...
       case 'del': {
			...
			if(!findData) return o.api.sendMessage("Box này hiện chưa kích hoạt thuê bot",t_id)
			data = data.filter(item=>item.t_id!==t_id)
			send(`✅ Đã xóa data của box thành công`)
			await save()
			};
			break;
        case 'check': {
        ...
        case 'loc': {
            ...
            send(`✅ Đã lọc và xóa ${removedCount} các nhóm mà bot đã rời khỏi danh sách kích hoạt thuê bot.`);
            break;
        }
        case 'list': {
			try{
...
========================================
➣ 𝐑𝐞𝐩𝐥𝐲: del + STT để xoá dữ liệu thuê bot nhóm.
➣ 𝐑𝐞𝐩𝐥𝐲: out + STT để thoạt nhóm.
➣ 𝐑𝐞𝐩𝐥𝐲: page + STT để xem các nhóm khác.
========================================`,o.event.threadID, (err, info)=>{
			...
			break;
        default:
            send(`Lệnh không hợp lệ. Vui lòng sử dụng: ${prefix}tb key [số tháng] hoặc ${prefix}tb list để xem danh sách.`);
            break;
    }
};
    ...
    if (event.body && event.body.startsWith('thangskidi_')) {
        ...
        if (keyIndex !== -1) {
            if (keysData[keyIndex].used) {
                api.sendMessage(`❎ Key này đã được sử dụng và không thể tái kích hoạt lại.`, threadID);
            } else if (isThreadActive) {
                api.sendMessage(`❎ Nhóm này đã được kích hoạt bot rồi và không thể sử dụng lại key khác để tái kích hoạt lại.`, threadID);
            } else {
                ...
                saveThuebotData();
                saveKeysData();
                api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Made by Tkhanh" : global.config.BOTNAME} | HSD: ${time_end}`, threadID, api.getCurrentUserID());
                ...
                api.getUserInfo(senderID, (err, ret) => {
                    ...
                    api.getThreadInfo(threadID, (err, info) => {
                        ...
                        const adminMessage = `🔑 Key: ${message} đã được kích hoạt bởi người dùng ${userName} (ID: ${senderID}) trong nhóm ${groupName} (ID: ${threadID})\n📆 Ngày kích hoạt: ${time_start}\n⏳ Hạn sử dụng: ${durationInDays} ngày, đến ${time_end}`;
...
📌 𝐂𝐨̀𝐧 ${(()=> {
    ...
    return `${days} 𝐧𝐠𝐚̀𝐲 ${hour} 𝐠𝐢𝐨̛̀ 𝐥𝐚̀ 𝐡𝐞̂́𝐭 𝐡𝐚̣𝐧.`;
            ...
            if (invalidIndexes.length > 0) {
                send(`STT không hợp lệ hoặc không tồn tại: ${invalidIndexes.join(', ')}.`);
                return;
            }
            ...
            saveThuebotData();
            send(`✅ Đã xóa các nhóm thuê bot có STT: ${indexes.map(i => i + 1).join(', ')} thành công.`);
    ...
    o.api.sendMessage(
        `📢 Thông báo gia hạn Bot cho nhóm ${threadName}\n\n` +
        `👤 Người gia hạn: Admin ${extenderName}\n` +
        `⏳ Số ngày mới được gia hạn: ${daysToAdd} ngày\n` +
        `📆 Hạn thuê bot mới: ${newEndDate}`, 
        threadIDToUpdate
    );
